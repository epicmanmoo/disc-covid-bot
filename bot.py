...
async def info(ctx):
    embed = discord.Embed(title="Information",
                          description="Use !covid for total stats and !covid <countryname> for a specific country. You can get a list of infected countries "
                                      "by doing !covid all <pagenumber>. If something doesn't work, feel free to message Segfault#9190.", color=0x000000)
...
async def covid(ctx, *args):
    if len(args) == 0:
        total_site_url = "https://corona.lmao.ninja/all"
        ...
        if len(args) > 2:
            ...
            return
        all_countries = []
        countries_url = "https://corona.lmao.ninja/countries"
        ...
        if not country_exists:
            ...
            date = date.strftime('%B %-dth, %Y. %I:%M %p.')
            embed.set_footer(text="Last Updated: " + date)
            await ctx.send(embed=embed)
